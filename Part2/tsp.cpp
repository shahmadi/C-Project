...
int Genetic::isValidSolution(vector<int>& solution)
{
	...
	for(int i = 0; i < graph->V; i++)
	{
			...
			int chk = graph->existsEdge(solution[i], solution[j]);
			
			
			if(chk > 0 && colors[j]==col){
				col++;
				break;
			}
		}
...
void Genetic::run()
{
	...
	for(int i = 0; i < graph->V; i++)
	{
			...
			int chk = graph->existsEdge(vec[i], vec[j]);
	...
	for(int i = 0; i < graph->V; i++)
		cout << vec[i] << " color "<<colors[i]<<" "<<endl;
	//cout << graph->initial_vertex;
	cout << "  Number of colors used " << population[0].second;
}
